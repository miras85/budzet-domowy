...
def get_archived_goals(db: Session = Depends(database.get_db), current_user: models.User = Depends(database.get_current_user)):
    ...
    return [{"id": g.id, "name": g.name, "target_amount": float(g.target_amount),
             ...
             "account_id": g.account_id} for g in goals]
...
def get_loans(db: Session = Depends(database.get_db), current_user: models.User = Depends(database.get_current_user)):
    ...
    for l in loans:
        all_loans.append({"id": l.id, "name": l.name, "total": float(l.total_amount),
            "remaining": float(l.remaining_amount), "monthly": float(l.monthly_payment),
        ...
        payment_info = {"loan_id": l.id, "name": l.name, "amount": float(l.monthly_payment),
            "date": str(l.next_payment_date), "days_until": days_until}
        if days_until < 0:
            overdue.append(payment_info); total_overdue += float(l.monthly_payment)
        elif days_until <= 7:
            urgent.append(payment_info); total_urgent += float(l.monthly_payment)
        elif days_until <= 30:
            upcoming.append(payment_info); total_upcoming += float(l.monthly_payment)

    return {"loans": all_loans,
        "alerts": {"overdue": overdue, "urgent": urgent, "upcoming": upcoming,
            "total_overdue": total_overdue, "total_urgent": total_urgent,
            "total_upcoming": total_upcoming, "has_alerts": len(overdue) > 0 or len(urgent) > 0},
...
def update_loan(loan_id: int, loan: schemas.LoanUpdate, db: Session = Depends(database.get_db), current_user: models.User = Depends(database.get_current_user)):
    ...
    db_loan.remaining_amount = loan.remaining_amount; db_loan.monthly_payment = loan.monthly_payment
    db_loan.next_payment_date = loan.next_payment_date; db.commit()
    return {"status": "updated"}
...
def update_account(account_id: int, acc: schemas.AccountUpdate, db: Session = Depends(database.get_db), current_user: models.User = Depends(database.get_current_user)):
    ...
    db_acc.balance = acc.balance; db_acc.is_savings = acc.is_savings
    db.commit()
    return {"status": "updated"}
...
def get_category_trend(cat_id: int, db: Session = Depends(database.get_db), current_user: models.User = Depends(database.get_current_user)):
    ...
    for i in range(5, -1, -1):
        ...
        if amount > 0:
            total_sum += amount; months_with_data += 1
        months = ["Sty", "Lut", "Mar", "Kwi", "Maj", "Cze",
                  "Lip", "Sie", "Wrz", "Paź", "Lis", "Gru"]
        data.append({"label": months[start.month - 1], "amount": amount,
    ...
    return {"category_name": category.name, "trend": data, "average": average,
        "suggested_limit": suggested_limit, "current_limit": float(category.monthly_limit)}
