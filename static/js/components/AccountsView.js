...
export default {
    props: ['accounts', 'userRole'],
    emits: ['create-account', 'edit-account', 'delete-account', 'trigger-import'],
    setup() { return { ...Utils }; },
    template: `
    <div class="px-6">
        <div class="flex justify-between items-center mb-6 mt-4">
            <h2 class="text-xl font-bold text-white">Moje konta</h2>
            <button v-if="userRole === 'admin'" @click="$emit('create-account')" class="text-blue-400 text-sm font-bold">+ Dodaj</button>
        </div>
        <div class="grid gap-4">
            <div v-for="acc in accounts" :key="acc.id" class="glass-panel p-5 rounded-2xl border-l-4" :class="acc.is_savings ? 'border-purple-500' : 'border-blue-500'">
