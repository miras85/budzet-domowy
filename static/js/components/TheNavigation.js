export default {
    props: ['currentTab', 'loanAlertsCount', 'userRole'],
    ...
    <div class="fixed bottom-0 left-0 right-0 glass-panel border-t border-slate-800 pb-safe safe-bottom z-50 h-16">
        <div class="nav-container">
            <div class="nav-group">
                <div @click="$emit('update:currentTab', 'dashboard')" :class="{active: currentTab === 'dashboard'}" class="nav-item">
                    ...
                    <span class="nav-label">Pulpit</span>
                </div>
                <div @click="$emit('update:currentTab', 'accounts')" :class="{active: currentTab === 'accounts'}" class="nav-item">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 nav-icon"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5H4.5a2.25 2.25 0 00-2.25 2.25v10.5a2.25 2.25 0 002.25 2.25z" /></svg>
                    <span class="nav-label">Konta</span>
                </div>
            </div>

            <!-- Przycisk + tylko dla admina -->
            <div class="fab-container">
                <button v-if="userRole === 'admin'" @click="$emit('open-add')" class="fab-btn group">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-8 h-8 group-active:rotate-90 transition-transform duration-200"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
                </button>
                <!-- Placeholder dla viewera żeby zachować layout -->
                <div v-else class="fab-btn opacity-20 cursor-not-allowed">
            ...
            <div class="nav-group">
                <div @click="$emit('update:currentTab', 'goals')" :class="{active: currentTab === 'goals'}" class="nav-item">
                    ...
                    <span class="nav-label">Cele</span>
                </div>
