...
createApp({
    ...
    computed: {
        ...
        groupedCategories() {
            ...
            this.filteredTransactions.forEach(tx => {
                ...
                const cat = tx.category || 'Bez kategorii';
                if (!groups[cat]) groups[cat] = { name: cat, total: 0, count: 0, transactions: [] };
    ...
    methods: {
        ...
        refreshAllData() { this.fetchData(); this.fetchAccounts(); this.fetchLoans(); this.fetchGoals(); this.fetchCategories(); this.fetchOverrides(); },
        
        async changePassword() {
            if(!this.security.oldPassword || !this.security.newPassword) return alert("Wpisz oba hasła");
            try {
                await this.authFetch('/api/users/change-password', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({ old_password: this.security.oldPassword, new_password: this.security.newPassword })
                });
                alert("Hasło zmienione pomyślnie!");
                this.security.oldPassword = ''; this.security.newPassword = '';
            } catch(e) { alert("Błąd: Sprawdź stare hasło"); }
        },
        async registerUser() {
            if(!this.security.newUsername || !this.security.newUserPass) return alert("Wpisz login i hasło");
            try {
                await this.authFetch('/api/users', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({ username: this.security.newUsername, password: this.security.newUserPass })
                });
                alert(`Użytkownik ${this.security.newUsername} dodany!`);
                this.security.newUsername = ''; this.security.newUserPass = '';
            } catch(e) { alert("Błąd: Taki użytkownik może już istnieć"); }
        },
        ...
        async fetchData() {
            ...
            try {
                this.dashboard = await (await this.authFetch(`/api/dashboard?offset=${this.periodOffset}`)).json();
